...
impl AstIndex {
    ...
    fn index_frames_with_components(
        ...
        let frame_re = regex::Regex::new(r"@frame\s*\([^)]+\)").unwrap();

        for mat in frame_re.find_iter(source) {
            let frame_start = mat.start();
            let frame_end = mat.end();
            let content = mat.as_str();

            // Find the component associated with this @frame annotation.
            // The @frame can either:
            // 1. Be contained within the component's span (doc comment is part of component span)
            // 2. Immediately precede the component (span.start > frame_end)
            let associated_component = doc
                .components
                .iter()
                // First, try to find a component that contains this frame annotation
                .find(|c| c.span.start <= frame_start && c.span.end >= frame_end)
                // If not found, find the component that immediately follows
                .or_else(|| {
                    doc.components
                        .iter()
                        .filter(|c| c.span.start > frame_end)
                        .min_by_key(|c| c.span.start)
                });

            // Use the component body's span.id if available, otherwise fall back to position-based
            let frame_id = if let Some(component) = associated_component {
                ...
                // No component found - fall back to position-based ID
                format!("frame-{}-{}", frame_start, frame_end)
            };
