...
import type { WorkspaceClient } from '@paperclip/workspace-client';
...
> = (
  ...
  let previewStream: AsyncIterableIterator<any> | null = null;
    ...
    async handleEvent(event: WorkspaceEvent, prevState: WorkspaceState) {
      switch (event.type) {
        ...
        case 'preview-requested': {
          // Start streaming preview updates
        ...
        case 'mutation-requested': {
          ...
          try {
            ...
            // Response events are dispatched by client event listeners
          } catch (error) {
            machine.dispatch({
              type: 'error-occurred',
              payload: {
        ...
        case 'outline-requested': {
          ...
          try {
            ...
            // Response event is dispatched by client event listener
          } catch (error) {
            machine.dispatch({
              type: 'error-occurred',
              payload: {
