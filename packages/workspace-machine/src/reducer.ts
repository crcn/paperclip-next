...
export const workspaceReducer: Reducer<WorkspaceEvent, WorkspaceState> = (
  ...
  switch (event.type) {
    ...
    case 'preview-updated': {
      ...
      // For now, if there's an initialize patch, use that VDOM
      // Otherwise keep existing VDOM (patch application will be done by rendering client)
    ...
    case 'outline-received': {
      ...
      // Assume outline has a file path in the first node or we use activeFilePath
    ...
    case 'error-occurred':
      // Could track errors in state if needed
      return state;
